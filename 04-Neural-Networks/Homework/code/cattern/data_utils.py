    ...
    for article_type in article_types:
        ...
        if create_val:
            files_train, files_val = train_test_split(
                files_train, random_state=0, test_size=0.1
            )
            val_words = generate_words(files_val)
            val_df = create_char_dataframe(val_words)
            val_df.to_csv(
                os.path.join(
                    output_path, "val", "df_best_{}_val.csv".format(article_type)
                ),
                index=False,
            )
        ...
        train_df.to_csv(
            os.path.join(
                output_path, "train", "df_best_{}_train.csv".format(article_type)
            ),
            index=False,
        )
        test_df.to_csv(
            os.path.join(
                output_path, "test", "df_best_{}_test.csv".format(article_type)
            ),
            index=False,
        )
